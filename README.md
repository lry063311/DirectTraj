...
### Key Features:
- ⚡ Real-Time Inference: Compresses inference latency to an unprecedented 4 ms by completely bypassing the iterative denoising paradigm.
- 🌍 Robust Structural Scalability: Synergizes with a pre-trained Graph Masked Autoencoder to extract coordinate-invariant routing rules. Using a hybrid "Frozen Prior + In-Domain Training" protocol, it ensures highly robust scalability across heterogeneous city topologies like Beijing T-Drive and San Francisco.
...
3. Single-Step Projection Head: Direct mapping from unconstrained noise to the valid trajectory space in a single forward pass, optimized with explicit kinematic consistency constraints.
